...
class Slider(Frame):
    LINE_COLOR = "#ffffff"  # White color for the line
    LINE_WIDTH = 6
    BAR_COLOR_INNER = "#90caf9"  # Light blue color for the inner bar
    BAR_COLOR_OUTTER = "#1976d2"  # Dark blue color for the outer bar
    BAR_RADIUS = 9
    ...
    def __init__(
        ...
        style = Style(self)
        style.configure("Slider.TFrame", background="#212121")  # Dark gray background
        self.configure(style="Slider.TFrame")

        self.canv = Canvas(self, height=self.canv_H, width=self.canv_W, bg="#212121", highlightthickness=0)  # Dark gray background, no border
        self.canv.pack()
    ...
    def __addBar(self, pos):
        ...
        if self.show_value:
            ...
            id_value = self.canv.create_text(
                x, y_value, text=format(value, Slider.DIGIT_PRECISION), fill="#ffffff"  # White color for the value text
            )
